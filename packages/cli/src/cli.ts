...
import { extractModelByDate, mergeContributions } from "./merge.js";
...
async function main() {
  ...
  program
    .command("model-dates [model]")
    .description("Show daily usage breakdown for a specific model (or all models)")
    .option("--light", "Use legacy CLI table output instead of TUI")
    .option("--json", "Output as JSON (for scripting)")
    .option("--opencode", "Show only OpenCode usage")
    .option("--claude", "Show only Claude Code usage")
    ...
    .option("--benchmark", "Show processing time")
    .option("--no-spinner", "Disable spinner (for AI agents and scripts - keeps stdout clean)")
    .action(async (model: string | undefined, options) => {
      await showModelByDateReport(model, options);
    });

  program
    .command("export")
    .description("Export usage data to a JSON file (for multi-machine merge)")
    .requiredOption("--output <file>", "Output file path")
    .option("--machine-id <id>", "Machine identifier label (default: hostname)")
  ...
  program
    .command("import <file>")
    .description("Import usage data from another machine and merge with local data")
    .option("--label <label>", "Label for the imported data source")
    .option("--dry-run", "Show what would be imported without saving")
    .option("--json", "Output merge summary as JSON")
    .option("--no-spinner", "Disable spinner (for AI agents and scripts - keeps stdout clean)")
  ...
  const knownCommands = ['monthly', 'models', 'model-dates', 'sources', 'headless', 'graph', 'export', 'import', 'wrapped', 'login', 'logout', 'whoami', 'submit', 'cursor', 'tui', 'pricing', 'help'];
...
// =============================================================================
// Model-by-Date Report
// =============================================================================

async function showModelByDateReport(
  modelFilter: string | undefined,
  options: FilterOptions & DateFilterOptions & { benchmark?: boolean; light?: boolean; json?: boolean; spinner?: boolean },
  extraOptions?: { spinner?: boolean }
) {
  const dateFilters = getDateFilters(options);
  const enabledSources = getEnabledSources(options);
  const onlyCursor = enabledSources?.length === 1 && enabledSources[0] === 'cursor';
  ...
  if (onlyCursor && !isCursorLoggedIn() && !hasCursorUsageCache()) {
    console.log(pc.red("\n  Error: Cursor authentication required."));
    console.log(pc.gray("  Run 'tokscale cursor login' to authenticate with Cursor.\n"));
    process.exit(1);
  }
  ...
  const dateRange = getDateRangeLabel(options);
  const title = modelFilter
    ? `Daily Usage for ${modelFilter}${dateRange ? ` (${dateRange})` : ""}`
    : `Daily Usage by Model${dateRange ? ` (${dateRange})` : ""}`;
  ...
  const useSpinner = !options.json && options.spinner !== false && extraOptions?.spinner !== false;
  const spinner = useSpinner ? createSpinner({ color: "cyan" }) : null;
  ...
  let graphData;
  try {
    graphData = await finalizeGraphAsync({
      localMessages: localMessages || emptyMessages,
      includeCursor: includeCursor && (cursorSync.synced || hasCursorUsageCache()),
      since: dateFilters.since,
      until: dateFilters.until,
  ...
  // Extract model-by-date data
  const modelByDate = extractModelByDate(graphData);

  // Filter by model name if specified
  let filteredModels: Map<string, Array<{ date: string; source: string; tokens: { input: number; output: number; cacheRead: number; cacheWrite: number; reasoning: number }; cost: number; messages: number }>>;

  if (modelFilter) {
    const lowerFilter = modelFilter.toLowerCase();
    // Find matching models (exact or partial)
    const matchingKeys = Array.from(modelByDate.keys()).filter(
      k => k.toLowerCase() === lowerFilter || k.toLowerCase().includes(lowerFilter)
    );

    if (matchingKeys.length === 0) {
      console.log(pc.yellow(`  No model found matching '${modelFilter}'.`));
      console.log(pc.gray("  Available models:"));
      for (const m of Array.from(modelByDate.keys()).sort()) {
        const entries = modelByDate.get(m)!;
        const totalCost = entries.reduce((s, e) => s + e.cost, 0);
        console.log(pc.gray(`    ${m} (${formatCurrency(totalCost)})`));
      }
      console.log();
      return;
    }

    filteredModels = new Map(
      matchingKeys.map(k => [k, modelByDate.get(k)!])
    );
  } else {
    filteredModels = modelByDate;
  }

  if (options.json) {
    const jsonData: Record<string, Array<{ date: string; source: string; tokens: { input: number; output: number; cacheRead: number; cacheWrite: number; reasoning: number }; cost: number; messages: number }>> = {};
    for (const [modelId, entries] of filteredModels) {
      jsonData[modelId] = entries;
    }
  ...
  // Display tables
  for (const [modelId, entries] of filteredModels) {
    const totalTokens = entries.reduce((s, e) => s + e.tokens.input + e.tokens.output + e.tokens.cacheRead + e.tokens.cacheWrite, 0);
    const totalCost = entries.reduce((s, e) => s + e.cost, 0);
    const totalMessages = entries.reduce((s, e) => s + e.messages, 0);

    console.log(pc.white(`  ${formatModelName(modelId)}`));
    console.log(pc.gray(`  ${entries.length} days, ${formatNumber(totalMessages)} messages, ${formatCurrency(totalCost)}`));
    console.log();

    const table = createUsageTable("Date");
    ...
    table.push(
      formatTotalsRow(
        entries.reduce((s, e) => s + e.tokens.input, 0),
        entries.reduce((s, e) => s + e.tokens.output, 0),
        entries.reduce((s, e) => s + e.tokens.cacheWrite, 0),
        entries.reduce((s, e) => s + e.tokens.cacheRead, 0),
        totalCost
      )
    );
    console.log(table.toString());
  ...
  if (options.benchmark) {
    console.log(pc.gray(`  Processing time: ${processingTime.toFixed(0)}ms (Rust native)`));
    if (cursorSync.attempted) {
      if (cursorSync.synced) {
        console.log(pc.gray(`  Cursor: ${cursorSync.rows} usage events synced`));
      } else {
        console.log(pc.yellow(`  Cursor: sync failed - ${cursorSync.error}`));
      }
    }
...
async function handleExportCommand(
  options: FilterOptions & DateFilterOptions & { output: string; machineId?: string; benchmark?: boolean; spinner?: boolean }
) {
  const dateFilters = getDateFilters(options);
  const enabledSources = getEnabledSources(options);
  const onlyCursor = enabledSources?.length === 1 && enabledSources[0] === 'cursor';
  ...
  const machineId = options.machineId || os.hostname();
  const exportData = {
    ...graphData,
    machineId,
  ...
  fs.writeFileSync(options.output, JSON.stringify(exportData, null, 2), "utf-8");

  spinner?.stop();

  console.log(pc.green(`\n  Export complete!`));
  console.log(pc.white(`  File: ${options.output}`));
  console.log(pc.gray(`  Machine: ${machineId}`));
  console.log(pc.gray(`  Days: ${graphData.contributions.length}`));
  console.log(pc.gray(`  Sources: ${graphData.summary.sources.join(", ")}`));
  console.log(pc.gray(`  Models: ${graphData.summary.models.length}`));
  console.log(pc.gray(`  Total cost: ${formatCurrency(graphData.summary.totalCost)}`));
...
async function handleImportCommand(
  filePath: string,
  options: { label?: string; dryRun?: boolean; json?: boolean; spinner?: boolean }
) {
  ...
  spinner?.start(pc.gray("Reading import file..."));
  ...
  try {
    ...
    spinner?.error(`Failed to read file: ${(e as Error).message}`);
    process.exit(1);
  }

  // Validate imported data has expected structure
  if (!importedData.contributions || !Array.isArray(importedData.contributions)) {
    spinner?.error("Invalid import file: missing 'contributions' array");
    process.exit(1);
  }

  if (!importedData.summary) {
    spinner?.error("Invalid import file: missing 'summary' object");
    process.exit(1);
  }

  const importLabel = options.label || importedData.machineId || path.basename(resolvedPath, ".json");

  spinner?.update(pc.gray("Scanning local session data..."));

  // Generate local graph data for comparison
  const homeDir = os.homedir();
  let localMessages: ParsedMessages | null = null;
  try {
    localMessages = await parseLocalSourcesAsync({
  ...
  let localGraphData;
  try {
    localGraphData = await finalizeGraphAsync({
      localMessages: localMessages || emptyMessages,
      includeCursor,
    });
  } catch {
    // Use empty local data
    localGraphData = {
      meta: { generatedAt: new Date().toISOString(), version: "0.0.0", dateRange: { start: "", end: "" } },
      summary: { totalTokens: 0, totalCost: 0, totalDays: 0, activeDays: 0, averagePerDay: 0, maxCostInSingleDay: 0, sources: [], models: [] },
      years: [],
      contributions: [],
    };
  }

  spinner?.update(pc.gray("Merging data..."));

  const mergeResult = mergeContributions(localGraphData, importedData, importLabel);
  ...
  if (options.json) {
    console.log(JSON.stringify({
      label: importLabel,
      localDays: mergeResult.localDays,
      importedDays: mergeResult.importedDays,
      mergedDays: mergeResult.mergedDays,
      duplicatesMerged: mergeResult.duplicatesMerged,
      totalCost: mergeResult.data.summary.totalCost,
      totalTokens: mergeResult.data.summary.totalTokens,
      models: mergeResult.data.summary.models,
      sources: mergeResult.data.summary.sources,
    }, null, 2));
    return;
  }

  console.log(pc.cyan("\n  Import Summary"));
  console.log(pc.gray(`  Source: ${resolvedPath}`));
  console.log(pc.gray(`  Label: ${importLabel}`));
  console.log();
  console.log(pc.white(`  Local data:    ${mergeResult.localDays} days`));
  console.log(pc.white(`  Imported data: ${mergeResult.importedDays} days`));
  console.log(pc.white(`  Merged result: ${mergeResult.mergedDays} days`));
  console.log(pc.white(`  Duplicates:    ${mergeResult.duplicatesMerged} entries summed`));
  console.log();
  console.log(pc.white(`  Total cost:  ${formatCurrency(mergeResult.data.summary.totalCost)}`));
  console.log(pc.white(`  Total tokens: ${formatNumber(mergeResult.data.summary.totalTokens)}`));
  console.log(pc.white(`  Models: ${mergeResult.data.summary.models.length}`));
  console.log();

  if (options.dryRun) {
    console.log(pc.yellow("  (dry run - no data saved)"));
    console.log();
    return;
  }

  // Save merged data to imports cache
  const importDir = path.join(homeDir, ".config", "tokscale", "imports");
  fs.mkdirSync(importDir, { recursive: true });
  const importFile = path.join(importDir, `${importLabel}-${Date.now()}.json`);
  fs.writeFileSync(importFile, JSON.stringify(mergeResult.data, null, 2), "utf-8");

  console.log(pc.green(`  Saved merged data to: ${importFile}`));
  console.log();
}
