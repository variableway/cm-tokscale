...
| <img width="48px" src=".github/assets/client-kimi.png" alt="Kimi" /> | [Kimi Code CLI](https://github.com/MoonshotAI/kimi-cli) | `~/.kimi/sessions/` (可通过 `KIMI_SHARE_DIR` 自定义) | ✅ 支持 |
...
- **交互式 TUI 模式** - 由 OpenTUI 驱动的精美终端 UI（默认模式）
  ...
  - 实时筛选和排序
  - 零闪烁渲染（原生 Zig 引擎）
- **多平台支持** - 跟踪 OpenCode、Claude Code、Codex CLI、Cursor IDE、Gemini CLI、Amp、Droid、OpenClaw 和 Kimi Code CLI 的使用情况
...
- **键盘导航**：
  ...
  - `1-9`：切换来源（OpenCode/Claude/Codex/Cursor/Gemini/Amp/Droid/OpenClaw/Kimi）
  - `p`：循环 9 种颜色主题
  - `r`：刷新数据
  - `e`：导出为 JSON
  - `q`：退出
...
- **年份筛选**：在年份之间导航
- **来源筛选**：按平台筛选（OpenCode、Claude、Codex、Cursor、Gemini、Kimi）
...
| Kimi Code CLI | `~/.kimi/` | `%USERPROFILE%\.kimi\` | 可通过 `KIMI_SHARE_DIR` 环境变量配置 |
...
位置：`~/.kimi/sessions/<workdir_hash>/<session_id>/wire.jsonl`（可通过 `KIMI_SHARE_DIR` 环境变量自定义）

包含 `StatusUpdate` 消息的 Wire 协议 JSONL 格式：
```json
{"timestamp": 1777274063.100584, "message": {"type": "StatusUpdate", "payload": {"context_usage": 0.045, "context_tokens": 11879, "max_context_tokens": 262144, "token_usage": {"input_other": 2663, "output": 23, "input_cache_read": 9216, "input_cache_creation": 0}, "message_id": "chatcmpl-xxx"}}}
```

模型名称从 `~/.kimi/config.toml` 中的 `default_model` 字段提取。
